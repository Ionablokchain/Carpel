...
class TokenType(Enum):
    ...
    FAT_ARROW   = auto()    # =>
    COLON_COLON = auto()    # ::
    UNDERSCORE  = auto()    # _ (only meaningful in patterns; the lexer emits
                            # it as a distinct token so the parser can tell
                            # `_` from a regular identifier)
    LPAREN      = auto()
    RPAREN      = auto()
    LBRACE      = auto()
    RBRACE      = auto()
...
class Lexer:
    ...
    def tokenize(self) -> List[Token]:
        while self.pos < len(self.source):
            ...
            if self._match2("=>"):
                self._add_with_width(TokenType.FAT_ARROW, "=>", 2); continue
            if self._match2("::"):
                self._add_with_width(TokenType.COLON_COLON, "::", 2); continue
    ...
    def _read_identifier_or_keyword(self) -> None:
        ...
        # `_` alone is the wildcard pattern token; `_name` is a normal ident.
        if text == "_":
            end_col = self.column
            self.tokens.append(Token(
