...
class VariantValue:
    """An enum variant at runtime.

    For unit variants `payload` is None.
    For tuple variants `payload` is a list of values, indexed positionally.
    For struct variants `payload` is a dict {field-name: value}.
    """
    __slots__ = ("enum_name", "variant_name", "kind", "payload")
    ...
    def __repr__(self) -> str:
        ...
        if self.kind == "tuple":
            body = ", ".join(_format(v) for v in self.payload)
            return f"{self.enum_name}::{self.variant_name}({body})"
        body = ", ".join(f"{k}: {_format(v)}" for k, v in self.payload.items())
        return f"{self.enum_name}::{self.variant_name} {{ {body} }}"
...
class Interpreter:
    ...
    def _eval(self, expr: Node) -> Any:
        ...
        if isinstance(expr, VariantConstruction):
            if expr.kind == "unit":
                return VariantValue(
                    expr.enum_name, expr.variant_name, "unit", None,
                )
            if expr.kind == "tuple":
                payload = [self._eval(a) for a in expr.tuple_args]
                return VariantValue(
                    expr.enum_name, expr.variant_name, "tuple", payload,
                )
            # struct variant
            payload = {fname: self._eval(fvalue)
                       for fname, fvalue, _ in expr.struct_fields}
            return VariantValue(
                expr.enum_name, expr.variant_name, "struct", payload,
            )
    ...
    def _exec_match(self, stmt: MatchStmt) -> None:
        ...
        for arm in stmt.arms:
            ...
            if self._pattern_matches(arm.pattern, value, bindings):
                ...
                try:
                    ...
                    self.scopes.pop()
                return
        # The type checker guarantees exhaustiveness, so reaching here is
        # an internal bug rather than a user error.
        raise CarpelRuntimeError(
            "internal: no match arm fired (type checker should have "
            "guaranteed exhaustiveness)"
        )

    def _pattern_matches(self, pat: Node, value: Any,
        ...
        if isinstance(pat, VariantPattern):
            if not isinstance(value, VariantValue):
                return False
            if (value.enum_name != pat.enum_name
                    or value.variant_name != pat.variant_name):
                return False
            if pat.kind == "unit":
                return True
            if pat.kind == "tuple":
                # The type checker guaranteed arity matches at compile time.
                for sub_pat, sub_val in zip(pat.tuple_sub, value.payload):
                    if not self._pattern_matches(sub_pat, sub_val, bindings):
                        return False
                return True
            # struct kind: iterate the sub-patterns; unmentioned fields
            # need not be bound (matches the type-checker's lenient rule).
            for fname, sub_pat, _ in pat.struct_sub:
                if not self._pattern_matches(
                        sub_pat, value.payload[fname], bindings):
                    return False
            return True
        raise CarpelRuntimeError(
